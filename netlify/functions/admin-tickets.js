...
function verifyToken(token) {
  try {
    ...
    if (!decoded.expiry || !decoded.sig) return null;
    if (Date.now() > decoded.expiry) return null;
    const role = decoded.role === 'staff' ? 'staff' : 'admin';
    const expected = crypto.createHmac('sha256', SECRET).update(String(decoded.expiry) + ':' + role).digest('hex');
    if (expected !== decoded.sig) return null;
    return { role };
  } catch (e) {
    return null;
  }
